...
export class Transactions {
    ...
    constructor(container, db, toast, confirmDialog) {
        if (!(container instanceof HTMLElement)) {
            throw new Error('Transactions: container inválido.');
        }
        if (!db) {
            throw new Error('Transactions: DB requerida.');
        }
        
        this._container = container;
        this._db = db;
        this._toast = toast;            // Guardamos la referencia
        this._confirmDialog = confirmDialog; // Guardamos la referencia
